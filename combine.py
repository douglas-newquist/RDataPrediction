...
def analyze(row, column, words, columns):
    text = row[column]
    words = [1 if word in text else 0 for word in words]
    return words + [row[c] for c in columns]


def main():
    reviews = pd.read_csv(sys.argv[1]).fillna('').astype(str)
    frequency = pd.read_csv(sys.argv[2])

    column = sys.argv[3]
    columns = ["points", "price"]
    # columns = ["country", "points", "price", "province", "region_1", "region_2", "taster_name", "variety", "winery"]
